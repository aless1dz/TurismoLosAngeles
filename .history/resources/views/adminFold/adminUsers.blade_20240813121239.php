...
<script>
    $(document).ready(function () {
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        if (row.length) {
                            ...
                            row.find('td:eq(1)').text(user.last_name);
                            row.find('td:eq(2)').text(user.role);
                            row.find('td:eq(3)').text(user.birthdate);
                            row.find('td:eq(4)').text(user.email);
                            row.find('td:eq(5)').text(user.created_at);
                            row.find('td:eq(6)').text(user.updated_at);
                        } else {
        ...
        $.ajax({
            url: '/users/search', // Ruta para la búsqueda de usuarios
            method: 'GET',
            data: { query: query },
            success: function (response) {
