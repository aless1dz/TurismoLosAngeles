...
<div class="container-fluid">
    ...
    <table id="userTable" class="table table-sm table-striped table-hover">
    <thead>
            <tr>
                <th>Nombre</th>
                <th>Apellido</th>
...
<script>
    ...
    $('#userForm').on('submit', function (e) {
        e.preventDefault();
        let id = $('#userId').val();
        let url = id ? `/users/${id}` : '/users'; // URL para POST o PUT
        let method = id ? 'PUT' : 'POST'; // Método HTTP
        let formData = $(this).serialize(); // Datos del formulario
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (row.length) {
                    row.replaceWith(createRow(response)); // Actualizar fila existente
                } else {
                    $('#userTable tbody').append(createRow(response)); // Añadir nueva fila
                }
    ...
    let id = row.data('id');
    console.log('ID del usuario:', id); // Agrega esta línea para depuración
    if (confirm('¿Estás seguro de que deseas eliminar este usuario?')) {
        $.ajax({
            ...
            error: function (xhr) {
                console.log('Error de AJAX:', xhr.responseText); // Agrega esta línea para depuración
                alert('Error: ' + xhr.responseText);
            }
        });
    }
    ...
    // Función para crear una fila de tabla HTML
