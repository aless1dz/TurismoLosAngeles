...
Mail::send('mail.confirmation_code', ['code' => $confirmationCode], function($message) use ($request) {
    ...
    public function confirmarCorreo($codigo)
    {
        ...
        if (!$user) {
            return redirect('/iniciar-sesion')->with('error', 'Código de confirmación inválido.');
        }
