...
public function destroy($idformalities)
{
    ...
    {
        if (!auth()->check()) {
            return redirect()->route('login')->with('error', 'Por favor, inicia sesión para ver tus citas.');
        }
    
        $userEmail = auth()->user()->email;
    
        // Ajusta los valores según los datos reales en la base de datos
        $citas = Formality::where('user_email', $userEmail)
                          ->where('form_type', 'cita') // Cambia a 'cita' si ese es el valor que esperas
                          ->get();
    
        $cotizaciones = Formality::where('user_email', $userEmail)
                                 ->where('form_type', 'cotizacion') // Cambia a 'cotizacion' si ese es el valor que esperas
                                 ->get();
    
        return view('auth.citasClientes', compact('citas', 'cotizaciones'));
    }
