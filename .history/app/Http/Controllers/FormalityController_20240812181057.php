...
class FormalityController extends Controller
{
    ...
    {
        // Obtén las formalidades del usuario autenticado
        $userId = Auth::id();
        $formalities = Formality::where('user_id', $userId)->get();

        // Pasa los datos a la vista
        return view('formalities.index', compact('formalities'));
    }

    public function cancel($id)
    {
        // Encuentra la formalidad
        $formality = Formality::findOrFail($id);

        // Verifica que la formalidad pertenece al usuario autenticado
        if ($formality->user_id !== Auth::id()) {
            abort(403, 'No tienes permiso para cancelar esta cita.');
        }

        // Marca la formalidad como cancelada
        $formality->status = 'cancelled';
        $formality->save();

        return redirect()->route('formalities.index')->with('success', 'Cita cancelada con éxito.');
    }
